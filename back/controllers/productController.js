...
exports.createProduct = async (req, res, next) => {
	try {
		...
		if (!nombre || costo == null || !restaurant) {
			return res.status(400).json({ status: 'fail', message: 'Missing required fields: nombre, costo, restaurant' });
		}
		if (typeof costo !== 'number' || costo < 0) {
			return res.status(400).json({ status: 'fail', message: 'Invalid costo value' });
		}
		const doc = await Product.create(req.body);
		return res.status(201).json({ status: 'success', data: { data: doc } });
	} catch (err) {
		if (err.code === 11000) {
			return res.status(400).json({ status: 'fail', message: 'Duplicate product for this restaurant (nombre + restaurant must be unique)' });
		}
...
exports.getProduct = async (req, res, next) => {
	try {
		...
		if (err.name === 'CastError') return next(new AppError('Invalid ID format', 400));
		return next(err);
	}
};
...
exports.deleteProduct = async (req, res, next) => {
	try {
		...
		const doc = await Product.findByIdAndUpdate(req.params.id, { active: false }, { new: true });
		if (!doc) {
			return res.status(404).json({ status: 'fail', message: 'No document found with that ID' });
		}
		return res.status(204).send();
	} catch (err) {
		return res.status(500).json({ status: 'error', message: 'Internal error', detail: err.message });
	}
};
