...
// Ensure the authenticated user is owner of the restaurant (for restaurant-admin)
exports.ensureRestaurantOwnership = async (req, res, next) => {
  ...
  const restaurantId = req.params.restaurantId || (req.body && req.body.restaurant) || req.params.id;
  // If there's no restaurant context, deny
  if (!restaurantId && req.user.restaurant == null) {
    return next(new AppError('No restaurant context to validate ownership', 400));
  }
  ...
  if (!req.user.restaurant || String(req.user.restaurant) !== String(targetRestaurantId)) {
    return next(new AppError('You can only manage resources for your own restaurant', 403));
  }
